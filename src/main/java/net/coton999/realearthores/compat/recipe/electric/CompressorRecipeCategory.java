...
public class CompressorRecipeCategory implements IRecipeCategory<CompressorRecipe> {
    public static final ResourceLocation UID = new ResourceLocation(RealEarthOres.MOD_ID, "compressor");
    ...
    public CompressorRecipeCategory(IGuiHelper guiHelper, int regularCookTime) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(REOBlocks.ELECTRIC_COMPRESSOR.get()));
    ...
    public Component getTitle() {
        return Component.literal("Compressing");
    }
    ...
    protected IDrawableAnimated getArrow(CompressorRecipe recipe) {
    ...
    protected void drawCookTime(CompressorRecipe recipe, GuiGraphics guiGraphics, int y) {
        int cookTime = recipe.getCookingTime();
        if (cookTime > 0) {
