...
public class CentrifugeRecipeCategory implements IRecipeCategory<CentrifugeRecipe> {
    public static final ResourceLocation UID = new ResourceLocation(RealEarthOres.MOD_ID, "centrifuge");
    ...
    public CentrifugeRecipeCategory(IGuiHelper guiHelper, int regularCookTime) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(REOBlocks.ELECTRIC_CENTRIFUGE.get()));
    ...
    public Component getTitle() {
        return Component.literal("Spinning");
    }
    ...
    protected IDrawableAnimated getArrow(CentrifugeRecipe recipe) {
    ...
    protected void drawCookTime(CentrifugeRecipe recipe, GuiGraphics guiGraphics, int y) {
        int cookTime = recipe.getCookingTime();
        if (cookTime > 0) {
