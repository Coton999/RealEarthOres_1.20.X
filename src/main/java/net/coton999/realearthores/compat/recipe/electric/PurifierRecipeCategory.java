...
public class PurifierRecipeCategory implements IRecipeCategory<PurifierRecipe> {
    public static final ResourceLocation UID = new ResourceLocation(RealEarthOres.MOD_ID, "purifier");
    ...
    public static final RecipeType<PurifierRecipe> PURIFIER_TYPE =
            new RecipeType<>(UID, PurifierRecipe.class);
    ...
    public PurifierRecipeCategory(IGuiHelper guiHelper, int regularCookTime) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(REOBlocks.ELECTRIC_PURIFIER.get()));
    ...
    public RecipeType<PurifierRecipe> getRecipeType() {
        return PURIFIER_TYPE;
    }

    @Override
    public Component getTitle() {
        return Component.literal("Purifying");
    }
    ...
    protected IDrawableAnimated getArrow(PurifierRecipe recipe) {
    ...
    public void draw(PurifierRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
    ...
    protected void drawCookTime(PurifierRecipe recipe, GuiGraphics guiGraphics, int y) {
        int cookTime = recipe.getCookingTime();
        if (cookTime > 0) {
