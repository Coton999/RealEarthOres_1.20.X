...
public class CrusherRecipeCategory implements IRecipeCategory<CrusherRecipe> {
    ...
    public CrusherRecipeCategory(IGuiHelper guiHelper, int regularCookTime) {
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(REOBlocks.ELECTRIC_CRUSHER.get()));
    ...
    protected IDrawableAnimated getArrow(CrusherRecipe recipe) {
    ...
    protected void drawCookTime(CrusherRecipe recipe, GuiGraphics guiGraphics, int y) {
        int cookTime = recipe.getCookingTime();
        if (cookTime > 0) {
