...
public class BuddingSapphireBlock extends AmethystBlock {
    ...
    public void randomTick(BlockState pState, ServerLevel pLevel, BlockPos pPos, RandomSource pRandom) {
        if (pRandom.nextInt(5) == 0) {
            ...
            if (canClusterGrowAtState(blockstate)) {
                block = REOBlocks.SMALL_SAPPHIRE_BUD.get();
            } else if (blockstate.is(REOBlocks.SMALL_SAPPHIRE_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = REOBlocks.MEDIUM_SAPPHIRE_BUD.get();
